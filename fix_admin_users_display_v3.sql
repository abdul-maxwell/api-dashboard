-- Fix Admin Users Display Issue - Version 3
-- This will fix the admin dashboard not showing all users without foreign key constraint issues
...
-- Step 3: Create test users safely (only pending signup users with NULL user_id)
DO $$
BEGIN
  ...
  -- Create test profile 2 (pending signup user)
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'pending@example.com') THEN
    INSERT INTO public.profiles (
      user_id,
      email,
      username,
...
-- Step 4: Check if there are any existing auth users we can use for testing
...
-- Step 9: Show auth users vs profiles comparison
SELECT 
  'Auth vs Profiles Comparison' as comparison_type,
  (SELECT COUNT(*) FROM auth.users) as auth_users_count,
  (SELECT COUNT(*) FROM public.profiles) as profiles_count,
  (SELECT COUNT(*) FROM public.profiles WHERE user_id IS NOT NULL) as linked_profiles_count,
  (SELECT COUNT(*) FROM public.profiles WHERE user_id IS NULL) as unlinked_profiles_count;
