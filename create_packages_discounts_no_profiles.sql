-- Create packages and discounts tables without requiring profiles table
-- This version uses simpler admin policies
...
CREATE TABLE IF NOT EXISTS public.packages (
  ...
  original_price_ksh DECIMAL(10,2),
...
-- Create basic policies (no admin restrictions for now)
...
-- Allow all authenticated users to manage packages and discounts for now
-- (Admin restrictions can be added later when profiles table is ready)
...
DROP POLICY IF EXISTS "Authenticated users can manage user discount usage" ON public.user_discount_usage;
CREATE POLICY "Authenticated users can manage user discount usage" ON public.user_discount_usage
  FOR ALL USING (auth.role() = 'authenticated');
